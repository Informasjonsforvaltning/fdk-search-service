...
class InformationModelTest: ApiTestContext() {
    ...
    private val SEARCH_QUERYS_HIT_ALL_FIELDS = listOf("title","keyword","theme","losTheme","description","publisher")
    private val INFORMATION_MODEL_PATH = "/informationmodels"

    @Test
    fun `search information models with at least one hit`() {
        val searchBody = mapper.writeValueAsString(SearchOperation(SEARCH_QUERY))
        val response = requestApi(INFORMATION_MODEL_PATH, port, searchBody, POST)
        Assertions.assertEquals(200, response["status"])

        val result: List<InformationModel> = mapper.readValue(response["body"] as String)
        Assertions.assertNotEquals(result.size, 0)
    }

    @Test
    fun `search information models with no hits`() {
    ...
    @Test
    fun `search information models with empty query`() {
