...
class EventSearchTest: ApiTestContext() {
    ...
    private val SEARCH_QUERYS_HIT_ALL_FIELDS = listOf("title","description")
    private val EVENT_PATH = "/events"

    @Test
    fun `search events with at least one hit`() {
        val searchBody = mapper.writeValueAsString(SearchOperation(SEARCH_QUERY))
        val response = requestApi(EVENT_PATH, port, searchBody, POST)
        Assertions.assertEquals(200, response["status"])

        val result: List<Event> = mapper.readValue(response["body"] as String)
        Assertions.assertNotEquals(result.size, 0)
    }

    @Test
    fun `search events with no hits`() {
    ...
    @Test
    fun `search events with empty query`() {
