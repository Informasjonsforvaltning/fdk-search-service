...
@Component
class InformationModelEventConsumer {
    ...
    fun listen(record: ConsumerRecord<String, RdfParseEvent>, ack: Acknowledgment) {
        ...
        try {
            ...
            if(event?.resourceType == RdfParseResourceType.INFORMATIONMODEL) {
                LOGGER.debug("Index information model - id: " + event.fdkId)
            }
            // ...
            ack.acknowledge();
        } catch (e: Exception) {
    ...
    companion object {
        private val LOGGER: Logger = LoggerFactory.getLogger(InformationModelEventConsumer::class.java)
    }
}